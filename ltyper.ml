...
let rec substitue_type v ts t = 
  match t with 
  | TVar(x) -> if x = v then TVar(v) else t
  | Arr(t1, t2) -> Arr (substitue_type v ts t1, substitue_type v ts t2)

let rec substitue_partout (v:string) (ts:ltype) (eqs: equa) : equa =  
  match eqs with 
  | [] -> []
  | (t1, t2) :: xs -> (substitue_type v ts t1, substitue_type v ts t2) 
...
let rec unification_etape eqs target = 
  ...
  | (t1, t2) :: xs when t1 = target || t2 = target -> 
      (unification_etape xs target) @ [(t1,t2)] 
  | (t1, t2) :: xs when t1 = t2 -> xs 
  | (TVar(x), t2) :: xs -> if not (occur_check x t2) then
        substitue_partout x t2 xs else failwith "error unification" 
...
let rec unification eqs timeout target = 
  if timeout <= 0 then failwith "timeout" else
    match eqs with 
    | [] -> failwith "error unification"
    | [x] -> x
    | x -> unification (unification_etape x target) (timeout - 1) target

let get_eq_target eq target = match eq with
    (t1, t2) -> if target = t1 then t2 else t1
